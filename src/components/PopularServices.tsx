...
const PopularServices = () => {
  ...
  const handleBookNow = (providerId: string, providerName: string) => {
    console.log(`Booking service from: ${providerName}`);
    toast({
      title: "Booking Initiated",
      description: `Starting booking process with ${providerName}`,
    });
    // Navigate to bookings page or show booking modal
    navigate('/bookings');
  };
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <Button 
                  className="w-full"
                  onClick={() => handleBookNow(provider.id, provider.business_name)}
                >
                  Book Now
                </Button>
              </div>
